...
function siparisTarihDegeri(item) {
    return tarihDegeriniOku(item, [
        "order.createdAt",
        "order.created_at",
        "order.createDate",
        "order.createdDate",
...
function siparisFiltreTarihDegeri(item) {
    if (aktifSiparisKuyrugu === "preparing") {
        return item.localPreparationStartedAt || siparisTarihDegeri(item);
    }
    if (aktifSiparisKuyrugu === "shipped") {
        return item.localShipmentShippedAt || item.localShipmentCarrierAcceptedAt || item.localShipmentUpdatedAt || siparisTarihDegeri(item);
    }
    return siparisTarihDegeri(item);
}

function tarihObjesineCevir(deger) {
...
function bugunTarihAnahtari(gunFarki = 0) {
...
function yerelSaatAnahtari(deger) {
    const tarih = tarihObjesineCevir(deger);
    if (!tarih) return "";
...
function siparisTarihFiltresineUyuyor(item) {
    ...
    if (aktifSiparisSaatBitis && (!saat || saat > aktifSiparisSaatBitis)) return false;
...
function siparisSiralamaUygula(liste) {
    ...
    return sirali.sort((a, b) => {
        const tarihA = tarihObjesineCevir(siparisFiltreTarihDegeri(a))?.getTime() || 0;
        const tarihB = tarihObjesineCevir(siparisFiltreTarihDegeri(b))?.getTime() || 0;
...
function siparisKuyrukListeleriniHesapla(liste) {
    ...
    const platformListesi = siparisSiralamaUygula(kuyrukListesi.filter(item =>
        ...
        && siparisTarihFiltresineUyuyor(item)
    ));
    ...
    result.innerHTML = `
        ...
        <div class="orderListControls">
            ...
            <label>
                ...
                <input id="orderTimeFrom" type="time" value="${temizle(aktifSiparisSaatBaslangic)}">
            </label>
            <label>
                <span>Bitiş saati</span>
                <input id="orderTimeTo" type="time" value="${temizle(aktifSiparisSaatBitis)}">
            </label>
            <label>
...
function urunEtiketiYazdir(etiket, tamamlandi) {
    ...
    frame.srcdoc = `
        ...
        <head>
            <meta charset="utf-8">
            <title>40x58 Ürün Barkodu</title>
            <style>
                ...
                * { box-sizing: border-box; }
                ...
                .barcodeLabel {
                    display: grid;
                    grid-template-rows: 9mm 7mm 10mm minmax(0, 1fr);
                    align-items: center;
                ...
                .barcodeLabelVariant .barcodeLabelColor,
                .barcodeLabelVariant .barcodeLabelLocation {
                    display: block;
                    border: 0;
                    border-radius: 0;
                    padding: 0;
                ...
                .barcodeLabelVariant .barcodeLabelSize {
                    display: block;
                    flex: 0 0 auto;
                    border: 0;
                    border-radius: 0;
...
function urunEtiketiPenceredeYazdir(etiket, tamamlandi) {
    const printWindow = window.open("", "zoomProductBarcodePrint", "popup,width=360,height=560");
    if (!printWindow) {
        tamamlandi();
    ...
    modal.innerHTML = `
        <div class="barcodePrintDialog" role="dialog" aria-modal="true" aria-labelledby="barcodePrintTitle">
            <div class="barcodePrintHeader">
                <div>
                    <p class="eyebrow">40 x 58 mm Zebra Etiketi</p>
...
async function kargoBarkodEtiketleriniYazdir(siparisVeyaListe) {
    ...
    const svgSatirlaraBol = (deger, satirUzunlugu, limit) => {
        const kelimeler = String(deger || "-").replace(/\s+/g, " ").trim().split(" ").filter(Boolean);
        const satirlar = [];
        kelimeler.forEach(kelime => {
            const son = satirlar[satirlar.length - 1] || "";
            if (!son) {
                satirlar.push(kelime);
            } else if (`${son} ${kelime}`.length <= satirUzunlugu) {
                satirlar[satirlar.length - 1] = `${son} ${kelime}`;
            } else {
                satirlar.push(kelime);
            }
        });
        if (!satirlar.length) satirlar.push("-");
        if (satirlar.length > limit) {
            satirlar.length = limit;
            satirlar[limit - 1] = `${satirlar[limit - 1].slice(0, Math.max(0, satirUzunlugu - 1))}…`;
        }
        return satirlar;
    };
    const svgMetinSatirlari = (satirlar, x, y, satirYuksekligi, boyut, kalinlik = 800, anchor = "middle") =>
        satirlar.map((satir, index) =>
            `<text x="${x}" y="${y + (index * satirYuksekligi)}" text-anchor="${anchor}" font-size="${boyut}" font-weight="${kalinlik}" font-family="Arial, sans-serif">${temizle(satir)}</text>`
        ).join("");
    ...
    const etiketler = printable.map(order => {
        ...
        const toplamAdet = (order.products || [])
            .filter(urun => !hizmetUrunuMu(urun))
            .reduce((toplam, urun) => toplam + urunAdedi(urun), 0);
        const musteriSatirlari = svgSatirlaraBol(musteriAdi(order), 22, 2);
        ...
        const etiketHtml = `
            <svg class="cargoBarcodeOnlyLabel" xmlns="http://www.w3.org/2000/svg" width="100mm" height="100mm" viewBox="0 0 1000 1000" preserveAspectRatio="none">
                <rect x="0" y="0" width="1000" height="1000" fill="#fff"/>
                ${svgMetinSatirlari(musteriSatirlari, 500, 72, 58, 54, 900)}
                <text x="500" y="180" text-anchor="middle" font-size="40" font-weight="900" font-family="Arial, sans-serif">Sipariş No: ${temizle(siparisKodu(order))}</text>
                <line x1="35" y1="215" x2="965" y2="215" stroke="#101828" stroke-width="6"/>
                ${urunHtml}
                <rect x="35" y="635" width="930" height="245" rx="14" fill="#fff" stroke="#101828" stroke-width="5"/>
                <svg x="65" y="662" width="870" height="185" preserveAspectRatio="xMidYMid meet">${svg.innerHTML}</svg>
                <text x="55" y="940" font-size="32" font-weight="900" font-family="Arial, sans-serif">${temizle(kargoFirmaEtiketi(order))}</text>
                <text x="945" y="940" text-anchor="end" font-size="30" font-weight="900" font-family="Arial, sans-serif">${temizle(urunler.length)} çeşit · ${temizle(toplamAdet)} adet</text>
            </svg>
        `;
        return { etiketHtml };
    });
    ...
    frame.srcdoc = `
        ...
        <body>
            ${etiketler.map(({ etiketHtml }) => etiketHtml).join("")}
        </body>
        </html>
    `;
    document.body.appendChild(frame);
...
result.addEventListener("change", function (event) {
    ...
    if (event.target.id === "orderTimeFrom") {
        aktifSiparisSaatBaslangic = event.target.value;
        aktifSiparisSayfasi = 1;
        listeGoster(aktifListe);
        return;
    }

    if (event.target.id === "orderTimeTo") {
        aktifSiparisSaatBitis = event.target.value;
        aktifSiparisSayfasi = 1;
        listeGoster(aktifListe);
        return;
    }
